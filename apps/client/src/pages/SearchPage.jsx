...
function SearchPage() {
  ...
  return (
    <section className="relative flex min-h-[calc(100vh-66px)] w-full flex-col overflow-hidden">
      {!isSearching ? (
        <div className="flex flex-1 items-center">
          <div className="flex flex-1 justify-start">
            <img src={heroLeft} className="h-full max-w-[90%] object-cover" alt="hero left" />
          </div>
          <div className="flex flex-1 justify-end">
            <img src={heroRight} className="h-full max-w-[90%] object-cover" alt="hero right" />
          </div>
        </div>
      ) : (
        <>
          <div>
            <img src={bgGradient} alt="bg-gradient" className="h-[90px] w-full object-cover" />
          </div>
        </>
      )}
      ...
      {showImages && <CollectionPhotos query={inputValue} />}
    </section>
  );
}
